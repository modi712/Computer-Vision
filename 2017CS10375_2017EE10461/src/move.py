...
image = cv2.imread('Images/3.jpg')	#show on 3
...
#    out = cv.VideoWriter('/home/nihar/Desktop/abc/middle.avi', -1, 20.0, (640,480))
    out = cv2.VideoWriter('./move.avi',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
    t = 0
#    frame = image   
        ...
        if len(matches) > MIN_MATCHES  and ONE:
            ...
            if (homography is not None )and (not args.model):
                try:
                    ...
                    projection1 = projection_matrix(camera_parameters, homography)
...
#                    print(homography)
                    projection = projection_matrix(camera_parameters, homography2)
#                    projection1[:,-1] = t*(projection[:,-1]-projection1[:,-1]) + projection1[:,-1]
#                    projection = t*(projection-projection1) + projection1
                    t += 0.02
                    # project cube or model
